  ...
  async with AsyncSessionLocal() as db:
    ...
    async with AsyncSessionLocal() as db:
        ...
        if video_path:
            ...
            logger.info(f"✅ Whisper transcript generated with 10-char line equalization (length: {len(transcript)} characters)")

            logger.info("🤖 Starting AI analysis for relevant segments")
            relevant_parts = get_most_relevant_parts_by_transcript(transcript)
...
async def process_video_task(task_id: str, raw_source: dict, user_id: str, font_family: str = "TikTokSans-Regular", font_size: int = 24, font_color: str = "#FFFFFF"):
    ...
    try:
        ...
        # Process video
        if video_path:
            logger.info(f"📊 Task {task_id}: Generating transcript with Whisper...")
            await update_task_status(task_id, "processing")
            transcript = get_video_transcript(video_path)
            ...
            logger.info(f"⏰ This may take 20-30 minutes for Ollama to process. Please be patient...")
            await update_task_status(task_id, "AI analyzing content for best clips...")
            
            # This is the critical call - it will wait up to 30 minutes
            relevant_parts = get_most_relevant_parts_by_transcript(transcript)
            logger.info(f"✅ AI analysis complete - found {len(relevant_parts.most_relevant_segments)} segments")

            # Validate that we got segments
            if not relevant_parts.most_relevant_segments or len(relevant_parts.most_relevant_segments) == 0:
                error_msg = f"AI analysis failed: {relevant_parts.summary}"
            ...
            if not clips_info or len(clips_info) == 0:
                error_msg = "Failed to generate video clips - no clips were created"
                logger.error(f"❌ Task {task_id}: {error_msg}")
                await update_task_status(task_id, "error")
                raise Exception(error_msg)
        ...
        # Mark as completed only if we have clips
